# Project method for the size based modelling package therMizer
...
# ocean temperature
# importing plankton to set background resource

#' project_therMizer method for the size based modelling
#' 
#' Runs the size-based model simulation and projects the size based model
#' through time. \code{project_therMizer} is called using an object of type
#' \linkS4class{therMizerParams}, an object that contains the effort of the
...
#' \linkS4class{therMizerSim} which can then be explored with a range of summary and
#' plotting methods. 
#' 
...
#'    dimensions of time step (or constant) x size. 
...
#'   the \code{therMizerParams} argument. By default the \code{cc_pp} slot of the
#'   \code{\link{therMizerParams}} argument is used.
...
#' names in the \code{therMizerParams} object. The values in the vector specify the
...
#' the \code{therMizerParams} object. The values in the vectory specify the 
...
#' order of species must be the same as in the \code{therMizerParams} argument. If
...
#' @return An object of type \code{therMizerSim}.
#' @export
#' @seealso \code{\link{therMizerParams}}
#' @examples
#' \dontrun{
...
#' }
setGeneric('project_therMizer', function(object, effort, ocean_temp, n_plank, ...) 
	standardGeneric('project_therMizer'))

# No effort is specified - default is to set an effort of 0 
# No ocean_temp is specified - default is set to a temp of 16.8 C 
# All other arguments passed as ...

#' Project without an effort or ocean_temp argument. 
#' @rdname project_therMizer
setMethod('project_therMizer', signature(object='therMizerParams', effort='missing', ocean_temp='missing', n_plank='missing'), 
	function(object, ...){
		res <- project_therMizer(object, effort=0, ocean_temp=16.8, ...) 
		return(res)
})

#' Project with a constant effort and ocean_temp. 
#' @rdname project_therMizer
setMethod('project_therMizer', signature(object='therMizerParams', effort='numeric', ocean_temp='numeric', n_plank='numeric'), 
	...
	if(!all.equal((t_max - floor(t_max / dt) * dt),0))
		...
		no_realms <- dim(object@exposure)[1] 
		if ((length(ocean_temp)>1) & (length(ocean_temp) != no_realms)) 
	...
	if(!all(gear_names %in% effort_gear_names)){
		gear_names_error_message <- paste("Gear names in the therMizerParams object (", paste(gear_names, collapse=", "), ") do not match those in the effort vector.", sep="")
		stop(gear_names_error_message)
	}
	# If more than 1 realm need to check that realm names match 
		realm_names <- dimnames(object@exposure)[[1]] 
	ocean_temp_realm_names <- names(ocean_temp) 
	if (length(ocean_temp) == 1 & is.null(ocean_temp_realm_names)){ 
		ocean_temp_realm_names <- realm_names 
	} 
	if(!all(realm_names %in% ocean_temp_realm_names)){ 
		realm_names_error_message <- paste("Realm names in the therMizerParams object (", paste(realm_names, collapse=", "), ") do not match those in the ocean_temp vector.", sep="") 
		stop(realm_names_error_message) 
	} 
		# Set up the effort, ocean_temp, and plankton arrays transposed so we can use the recycling rules 
		plank_sz <- length(object@w_full)
		plank_nm <- names(object@w_full) 
	time_dimnames <- signif(seq(from=1,to=t_max,by=dt),3)
		...
		ocean_temp_array <- t(array(ocean_temp, dim=c(no_realms,length(time_dimnames)), dimnames=list(realm=ocean_temp_realm_names,time=time_dimnames))) 
		n_plank_array <- t(array(n_plank, dim=c(plank_sz,length(time_dimnames)),dimnames=list(plank_nm,time=time_dimnames))) 
		res <- project_therMizer(object,effort_array, ocean_temp_array, n_plank_array, dt=dt, ...) 
		return(res)
})

#' Project with time varying effort and ocean temperature
#' @rdname project_therMizer
setMethod('project_therMizer', signature(object='therMizerParams', effort='array', ocean_temp='array', n_plank='array'), 
	function(object, effort, ocean_temp, n_plank, t_save=1, dt=0.1, initial_n=get_initial_n(object), initial_n_pp=object@cc_pp, ...){ 
		validObject(object)
		# Check that number and names of gears in effort array is same as in therMizerParams object
		no_gears <- dim(object@catchability)[1]
		if(dim(effort)[2] != no_gears){
			no_gears_error_message <- paste("The number of gears in the effort array (length of the second dimension = ", dim(effort)[2], ") does not equal the number of gears in the therMizerParams object (", no_gears, ").", sep="")
			stop(no_gears_error_message)
		}
		gear_names <- dimnames(object@catchability)[[1]]
		if(!all(gear_names %in% dimnames(effort)[[2]])){
			gear_names_error_message <- paste("Gear names in the therMizerParams object (", paste(gear_names, collapse=", "), ") do not match those in the effort array.", sep="")
			stop(gear_names_error_message)
		}
		# Sort effort array to match order in therMizerParams
		effort <- effort[,gear_names, drop=FALSE]

		# Check that number and names of realms in ocean_temp array is same as in therMizerParams object 
		no_realms <- dim(object@exposure)[1] 
		if(dim(ocean_temp)[2] != no_realms){ 
			no_realms_error_message <- paste("The number of realms in the ocean_temp array (length of the second dimension = ", dim(ocean_temp)[2], ") does not equal the number of realms in the therMizerParams object (", no_realms, ").", sep="") 
			stop(no_realms_error_message) 
		} 
		realm_names <- dimnames(object@exposure)[[1]] 
		if(!all(realm_names %in% dimnames(ocean_temp)[[2]])){ 
			realm_names_error_message <- paste("Realm names in the therMizerParams object (", paste(realm_names, collapse=", "), ") do not match those in the ocean_temp array.", sep="") 
			stop(realm_names_error_message) 
		} 
		# Sort ocean_temp array to match order in therMizerParams
		ocean_temp <- ocean_temp[,realm_names, drop=FALSE] 
		
		# Blow up time dimensions of effort, ocean_temp, and n_plank arrays (
		# i.e. effort might have been passed in using time steps of 1, but actual dt = 0.1, so need to blow up
		# NOTE: This assumes that effort, ocean_temp, and n_plank have the same time step. 
		# NOTE: This fills ocean_temp and n_plank with the time steps from effort.
		...
		if (is.null(dimnames(ocean_temp)[[1]])){ 
			stop("The time dimname of the ocean_temp argument must be numeric.") 
		} 
		if (any(is.na(as.numeric(dimnames(ocean_temp)[[1]])))){ 
			stop("The time dimname of the ocean_temp argument must be numeric.")
		} 
		if (is.null(dimnames(n_plank)[[1]])){ 
			stop("The time dimname of the n_plank argument must be numeric.")
		} 
		if (any(is.na(as.numeric(dimnames(n_plank)[[1]])))){ 
			stop("The time dimname of the n_plank argument must be numeric.") 
		} 
		time_effort <- as.numeric(dimnames(effort)[[1]])
		t_max <- time_effort[length(time_effort)]
		# Blow up effort and ocean_temp so that rows are dt spaced 
		...
		ocean_temp_dt <- t(array(NA, dim = c(length(time_effort_dt), dim(ocean_temp)[2]), dimnames=list(time = time_effort_dt, dimnames(ocean_temp)[[2]]))) 
		n_plank_dt <- t(array(NA, dim = c(length(time_effort_dt), dim(n_plank)[2]), dimnames=list(time = time_effort_dt, dimnames(n_plank)[[2]]))) 
		for (i in 1:length(time_effort)){
			effort_dt[,time_effort_dt >= time_effort[i]] <- effort[i,]
			ocean_temp_dt[,time_effort_dt >= time_effort[i]] <- ocean_temp[i,] 
			n_plank_dt[,time_effort_dt >= time_effort[i]] <- n_plank[i,] 
		}
		effort_dt <- t(effort_dt)
		ocean_temp_dt <- t(ocean_temp_dt) 
		n_plank_dt <- t(n_plank_dt) 

		# Make the therMizerSim object with the right size
		...
		sim <- therMizerSim(object, t_dimnames = t_dimnames) 
		# Fill up the effort array
		sim@effort[] <- effort_dt[t_dimnames_index,]
		sim@ocean_temp[] <- ocean_temp_dt[t_dimnames_index,] 
		sim@n_plank[] <- n_plank_dt[t_dimnames_index,] 
		...
		for (i_time in 1:t_steps){
			...
			feeding_level <- getFeedingLevel(sim@params, n=n, n_pp=n_pp, ocean_temp=ocean_temp_dt[i_time,], phi_prey=phi_prey) 
			...
			m2 <- getM2(sim@params, n=n, n_pp=n_pp, ocean_temp=ocean_temp_dt[i_time,], phi_prey=phi_prey, feeding_level=feeding_level, pred_rate=pred_rate) 
			...
			e <- getEReproAndGrowth(sim@params, n=n, n_pp=n_pp, ocean_temp=ocean_temp_dt[i_time,], feeding_level=feeding_level) 
			...
			# Reading in the background spectrum at each time step 
			n_pp <- n_plank_dt[i_time,] 
...
#' returned population can be passed to the \code{project_therMizer} method.
#' 
#' @param params The model parameters. An object of type \code{therMizerParams}.
...
get_initial_n<- function(params, n0_mult = NULL, a = 0.35){
	if (!is(params,"therMizerParams"))
		stop("params argument must of type therMizerParams")
