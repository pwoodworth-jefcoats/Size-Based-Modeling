...
#' 
#' This functions creates a \code{\linkS4class{therMizerParams}} object so that
...
#' \code{therMizerParams} object) is set to 0. Standard metabolism has been turned 
...
#' selected.
#' 
#' The resulting \code{therMizerParams} object can be projected forward using 
#' \code{project_therMizer()} like any other \code{therMizerParams} object. When projecting 
...
#' @param ... Other arguments to pass to the \code{therMizerParams} constructor.
#' @export
#' @return An object of type \code{\linkS4class{therMizerParams}}
...
set_community_model <- function(max_w = 1e6,
	...
	com_params <- therMizerParams(com_params_df, p=p, n=n,q=q, lambda = lambda, 
...
#' 
#' This functions creates a \code{therMizerParams} object so that trait-based-type 
...
#' \code{therMizer}. All the species-specific parameters are the same, except for
...
#' @seealso \linkS4class{therMizerParams}
...
#' sim <- project_therMizer(trait_params, effort = 0, t_max = 50, dt=0.2,
...
#' sim <- project_therMizer(params_gear, t_max = 20, effort = c(Industrial = 1, Other = 0))
#' }
