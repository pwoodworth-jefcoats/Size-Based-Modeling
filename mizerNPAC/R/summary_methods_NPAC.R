...
#' the \code{MizerSim_NPAC} class. SSB is calculated as the total mass of all mature
#' individuals.
#' 
...
#' Calculate the SSB from a \code{MizerSim_NPAC} object
#' @rdname getSSB
setMethod('getSSB', signature(object='MizerSim_NPAC'),
	function(object){
...
#' \code{MizerSim_NPAC} class within user defined size limits. The default option is
...
#' Calculate the total biomass of each species from a \code{MizerSim_NPAC} object
#' @rdname getBiomass
setMethod('getBiomass', signature(object='MizerSim_NPAC'),
	function(object, ...){
...
#' Calculate the total abundance from a \code{MizerSim_NPAC} object
#' @rdname getN
setMethod('getN', signature(object='MizerSim_NPAC'),
	function(object, ...){
			size_range <- get_size_range_array(object@params,...)
...
#' getYieldGear(sim)
#' }
setGeneric('getYieldGear', function(object, ...) # PW
	standardGeneric('getYieldGear'))

#' Calculate the total yield per gear from a \code{MizerSim_NPAC} object
#' @rdname getYieldGear
setMethod('getYieldGear', signature(object='MizerSim_NPAC'),
	function(object, ...){ # PW
		size_range <- get_size_range_array(object@params,...) # PW
		# biomass less the first time step
		biomass <- sweep(object@n,3,object@params@w * object@params@dw, "*")[-1,,,drop=FALSE]
		biomass_sz <- sweep(biomass,c(2,3),size_range,"*") # PW
		f_gear <- getFMortGear(object)
		f_gear_sz <- sweep(f_gear,c(3,4),size_range,"*") # PW
		yield_species_gear <- apply(sweep(f_gear_sz,c(1,3,4),biomass_sz,"*"),c(1,2,3),sum) # PW
...
#' data(inter_NPAC)
#' params <- MizerParams_NPAC(NS_species_params_gears_NPAC, inter_NPAC)
#' sim <- project_NPAC(params, effort=1, t_max=10)
#' y <- getYield(sim)
#' }
setGeneric('getYield', function(object, ...) # PW
	standardGeneric('getYield'))

#' Calculate the total yield of each species from a \code{MizerSim_NPAC} object
#' @rdname getYield
setMethod('getYield', signature(object='MizerSim_NPAC'),
	function(object, ...){ # PW
		# biomass less the first time step
		yield_gear_species <- getYieldGear(object, ...) # PW
...
#' Summarize MizerParams_NPAC object 
#'
...
#' summary(params)
#' }
setMethod("summary", signature(object="MizerParams_NPAC"),
	function(object, ...){
...
#' Summarize MizerSim_NPAC object 
#'
#' Outputs a general summary of the structure and content of the object
#' @param object A \code{MizerSim_NPAC} object.
#' @param ... Other arguments (currently not used).
#'
#' @examples
#' \dontrun{
...
#' data(inter_NPAC)
#' params <- MizerParams_NPAC(NS_species_params_gears_NPAC,inter_NPAC)
#' sim <- project_NPAC(params, effort=1, t_max=5)
#' summary(sim)
#' }
setMethod("summary", signature(object="MizerSim_NPAC"),
	function(object, ...){
		#cat("An object of class \"", as.character(class(object)), "\" with:\n", sep="")
		cat("An object of class \"", as.character(class(object)), "\" \n", sep="")
		cat("Parameters:\n")
...
#' 
#' Calculates the proportion of large fish through time in the \code{MizerSim_NPAC}
...
#' Calculate the proportion of large fish from a \code{MizerSim_NPAC} object.
#' @rdname getProportionOfLargeFish
setMethod('getProportionOfLargeFish', signature(object='MizerSim_NPAC'),
...
#' Calculate the mean weight of the community from a \code{MizerSim_NPAC} object.
#' @rdname getMeanWeight
setMethod('getMeanWeight', signature(object='MizerSim_NPAC'),
...
#' Calculate the mean maximum weight of the community from a \code{MizerSim_NPAC} object.
#' @rdname getMeanMaxWeight
setMethod('getMeanMaxWeight', signature(object='MizerSim_NPAC'),
...
#' data(inter_NPAC)
#' params <- MizerParams_NPAC(NS_species_params_gears_NPAC, inter_NPAC)
#' sim <- project_NPAC(params, effort=1, t_max=40, dt = 1, t_save = 1)
...
#' Calculate the slope of the community abundance from a \code{MizerSim_NPAC} object.
#' @rdname getCommunitySlope
setMethod('getCommunitySlope', signature(object = 'MizerSim_NPAC'),
	function(object, species = 1:nrow(object@params@species_params),
			 biomass = TRUE, ...) {
		check_species(object,species)
		size_range <- get_size_range_array(object@params,...)
