...
# Modified to include temperature by:
...
valid_MizerParams_NPAC <- function(object) {
	...
	if(!all(c(
		...
		length(dim(object@exposure))) == 2)){ # PW
			msg <- "psi, intake_max, search_vol, activity, std_metab, interaction, catchability, and exposure must all be two dimensions" # PW
			errors <- c(errors, msg)
	}
	# 3D arrays
	if(!all(c(
		...
		length(dim(object@ontogenetic_migration))) == 3)){  # PW
			msg <- "pred_kernel, selectivity, and ontogenetic_migration must be three dimensions" # PW
			errors <- c(errors, msg)
	}
	...
	if(!all(c(
		...
		dim(object@ontogenetic_migration)[2], # PW
		dim(object@exposure)[2], # PW
		dim(object@interaction)[1],
		dim(object@interaction)[2]) == 
			dim(object@species_params)[1])){
			msg <- "The number of species in the model must be consistent across the species_params, psi, intake_max, search_vol, activity, pred_kernel, interaction (dim 1), selectivity, catchability, ontogenetic_migration, exposure, and interaction (dim 2) slots" # PW
			errors <- c(errors, msg)
	}
	...
	if(!all(c(
		...
		dim(object@ontogenetic_migration)[3]) == # PW
			length_w)){
			msg <- "The number of size bins in the model must be consistent across the w, psi, intake_max, search_vol, activity, pred_kernel (dim 2), selectivity, and ontogenetic_migration (dim 3) slots" # PW
			errors <- c(errors, msg)
	}
	...
	# Check number of realms (PW)
	if(!isTRUE(all.equal(dim(object@ontogenetic_migration)[1], dim(object@exposure)[1]))){ # PW
		msg <- "The number of oceanic realms must be consistent across the ontogenetic_migration and exposure (dim 1) slots" # PW
	...
	if(!all(c(
		...
		names(dimnames(object@ontogenetic_migration))[2], # PW
		names(dimnames(object@exposure))[2]) == "sp")){ # PW
			msg <- "Name of first dimension of psi, intake_max, search_vol, std_metab, activity and pred_kernel and the second dimension of selectivity, catchability, ontogenetic_migration, and exposure must be 'sp'" # PW
			errors <- c(errors, msg)
		}
	...
	if(!all(c(
		...
		names(dimnames(object@ontogenetic_migration))[3]) == "w")){ # PW
			msg <- "Name of second dimension of psi, intake_max, search_vol, std_metab, activity and third dimension of selectivity and ontogenetic_migration must be 'w'" # PW
			errors <- c(errors, msg)
		}
	...
	if(!all(c( # PW
		  names(dimnames(object@ontogenetic_migration))[1], # PW
		  names(dimnames(object@exposure))[1]) == "realm")){ # PW
		msg <- "Name of the first dimension of ontogenetic_migration and exposure must be 'realm'" # PW
		errors <- c(errors, msg)
	}
	...
	if(!all(c(
		...
		dimnames(object@ontogenetic_migration)[[2]], # PW
		dimnames(object@exposure)[[2]], # PW
		dimnames(object@interaction)[[1]],
		dimnames(object@interaction)[[2]]) ==
			object@species_params$species)){
			msg <- "The species names of species_params, psi, intake_max, search_vol, std_metab, activity, pred_kernel, selectivity, catchability, ontogenetic_migration, exposure, and interaction must all be the same" # PW
			errors <- c(errors, msg)
	}
	# Check dimnames of w
	if(!all(c(
		...
		dimnames(object@selectivity)[[3]]) == # PW
			dimnames(object@ontogenetic_migration)[[3]])){ # PW
			msg <- "The size names of psi, intake_max, search_vol, std_metab, activity, pred_kernel, selectivity, and ontogenetic_migration must all be the same" # PW
			errors <- c(errors, msg)
	}
	...
	# Check dimnames of realm (PW)
	if(!isTRUE(all.equal( # PW
		dimnames(object@exposure)[[1]], # PW
		dimnames(object@ontogenetic_migration)[[1]]))){ # PW
			msg <- "The realm names of ontogenetic_migration and exposure must all be the same" # PW
			errors <- c(errors, msg) # PW
	} # PW
...
#' 
#' @note The \code{MizerParams_NPAC} class is fairly complex with a large number of
...
#' @seealso \code{\link{project_NPAC}} \code{\link{MizerSim_NPAC}}
#' @export
setClass(
	"MizerParams_NPAC",
	representation(
		...
		selectivity = "array",
		catchability = "array",
		ontogenetic_migration = "array", # PW
		exposure = "array" # PW
	),
	prototype = prototype(
		w = NA_real_,
		...
		ontogenetic_migration = array( # PW
			NA, dim = c(1,1,1), dimnames = list(realm = NULL, sp = NULL, w = NULL) # PW
		), # PW
		exposure = array( # PW
			NA, dim = c(1,1), dimnames = list(realm = NULL, sp = NULL) # PW
		) # PW
	),
	validity = valid_MizerParams_NPAC
)


# Generic constructor

#' Constructors for objects of \code{MizerParams_NPAC} class
#'
#' Constructor method for the \linkS4class{MizerParams_NPAC} class. Provides the
#' simplest way of making a \code{MizerParams_NPAC} object to be used in a
#' simulation.
#' 
...
#'   dimnames of the interaction matrix in the returned \code{MizerParams_NPAC}
...
#'   Default is \code{species_names}. (PW)
#' @param ... Additional arguments.
#'
#' @return An object of type \code{MizerParams_NPAC}
#' @note The only essential argument to the \code{MizerParams_NPAC} constructor is a
...
#'   data frame, they will be automatically added when the \code{MizerParams_NPAC}
...
#' \code{MizerParams_NPAC} object with the correct dimensions. It is not recommended
#' that this method is used by users.
#' @seealso \code{\link{project_NPAC}} \linkS4class{MizerSim_NPAC}
#' @export
#' @examples
#' data(NS_species_params_gears_NPAC)
#' data(inter_NPAC)
#' params <- MizerParams_NPAC(NS_species_params_gears_NPAC, inter_NPAC)
setGeneric('MizerParams_NPAC', function(object, interaction, ...)
	standardGeneric('MizerParams_NPAC'))

#' Basic constructor with only the number of species as dispatching argument
#' 
#' Only really used to make MizerParams_NPAC of the right size and shouldn't be used
#' by user
#' @rdname MizerParams_NPAC
setMethod('MizerParams_NPAC', signature(object='numeric', interaction='missing'),
	function(object, min_w = 0.001, max_w = max(object$w_inf)*1.1, no_w = 100, min_w_pp = 1e-14, no_w_pp = round(no_w)*0.3, species_names=1:object, gear_names=species_names, realm_names=species_names){ # PW
		#args <- list(...)
		...
		ontogenetic_migration <- array(0, dim=c(length(realm_names), object, no_w), dimnames=list(realm=realm_names, sp=species_names, w=signif(w,3))) # PW
		exposure <- array(0, dim=c(length(realm_names), object), dimnames = list(realm=realm_names, sp=species_names)) # PW
		...
		# The project_NPAC method uses the columns species z0 alpha erepro
		...
		res <- new("MizerParams_NPAC",
			...
			ontogenetic_migration=ontogenetic_migration, exposure=exposure, # PW
...
#' @rdname MizerParams_NPAC
setMethod('MizerParams_NPAC', signature(object='data.frame', interaction='matrix'),
	function(object, interaction, n = 2/3, p = 0.7, q = 0.8, r_pp = 10, 
			  kappa = 1e11, lambda = (2+q-n), w_pp_cutoff = max(object$w_inf)*10.1,# PW changed from 10 
		...
		# If no realm_name column in object, then named after species (PW)
		if(!("realm" %in% colnames(object))) # PW
			object$realm <- object$species # PW
		no_realm <- length(unique(object$realm)) # PW
	...
	# Should this be a column? Or just an argument?
		...
		if(!("erepro" %in% colnames(object)))
			object$erepro <- 1
		# If no sel_func column in species_params, set to 'knife_edge' (PW)
		...
		# If no mig_func column in species_params, set to 'mig_window' (PW)
		if(!("mig_func" %in% colnames(object))){ # PW
		message("\tNote: No mig_func column in species data frame. Setting ontogenetic migration to be 'mig_window' for all species.") # PW
			object$mig_func <- 'mig_window' # PW
		# Set default migration sizes (PW)
		if(!("mig_min_size" %in% colnames(object))){ # PW
			message("\tNote: No mig_min_size column in species data frame. Setting minimum exposure size to w_min.") # PW
			object$mig_min_size <- object$w_min # PW 
		} # PW
		if(!("mig_max_size" %in% colnames(object))){ # PW 
			message("\tNote: No mig_max_size column in species data frame. Setting maximum exposure size to w_inf.") # PW
			object$mig_max_size <- object$w_inf # PW
		} # PW
	} # PW
		# If no catchability column in species_params, set to 1
		if(!("catchability" %in% colnames(object)))
			object$catchability <- 1
		# If no exposure column in species_params, set to 1 (PW)
		if(!("exposure" %in% colnames(object))) # PW
			object$exposure <- 1 # PW
	...
	# Determine T_opt and R(T_opt) # PW
	if(!("temp_min" %in% colnames(object))){ # PW
		message("\tNote: No minimum temperature in species data frame so using 16.8.") # PW
		object$temp_min <- 16.8 # PW
	} # PW
	if(!("temp_max" %in% colnames(object))){ # PW
		message("\tNote: No maximum temperature in species data frame so using 16.8.") # PW
		object$temp_max <- 16.8 # PW
	} # PW
	if(!("temp_scale" %in% colnames(object))){ # PW
		stop("\tNote: No value to scale the temperature effect in species data frame.  This is bad.  Go back and add this.") # PW
	} # PW
		...
		res <- MizerParams_NPAC(no_sp, species_names=object$species, 
		...
		object$w_min_idx <- as.vector(
			tapply(object$w_min,1:length(object$w_min),
					...
					#Rewriting this bit because right now it indexes the size smaller than w_min (PW)
					function(w_min,wx) # PW
					if(w_min == 0.001){ # PW
						max(which(wx<=w_min)) # PW
					} else { # PW
						max(which(wx<=w_min)) + 1 # PW
					} # PW
					,wx=res@w)) # PW
	...
	combn <- NULL # Stupid hack to pass documentation check
		...
		res@cc_pp[] <- kappa*res@w_full^(-lambda) # the resource carrying capacity - one for each mp and m (130 of them) # PW
		...
		# Set temperature effect parameters: ontogenetic_migration and exposure (PW)
		# At the moment, each species inhabits only 1 realm, so in species_params (PW)
		# there are the columns: realm_name and mig_func. (PW)
		# BEWARE! This routine assumes that each species is exposed to only one realm (PW)
		# So we can just go row by row through the species parameters (PW)
		# However, I hope to improve this in the future (PW)
		for (te in 1:nrow(object)){ # PW
			# Do ontogenetic_migration first (PW)
			# get szzs (PW)
			szz <- names(formals(as.character(object[te,'mig_func']))) # PW
			# lop off w as that is always the first argument of the migration functions (PW)
			szz <- szz[!(szz %in% "w")] # PW
			if(!all(szz %in% colnames(object))) # PW
				stop("All of the arguments needed for the migration function are not in the parameter dataframe") # PW
			# Check that there is only one column in the object with the same name (PW)
			# Check that column of arguments exists (PW)
			rap <- c(w=list(res@w),as.list(object[te,szz])) # PW
			mig <- do.call(as.character(object[te,'mig_func']), args=rap) # PW
			# Dump mig in the right place (PW)
			res@ontogenetic_migration[as.character(object[te,'realm']), te, ] <- mig # PW
			# Now do exposure (PW)
			res@exposure[as.character(object[te,'realm']), te] <- object[te,"exposure"] #PW
		} # PW
		
		# Remove exposure from species data.frame, now storedin slot (PW)
		res@species_params <- res@species_params[,-which(names(res@species_params)=="exposure")] # PW
...
#' @rdname MizerParams_NPAC
setMethod('MizerParams_NPAC', signature(object='data.frame', interaction='missing'),
	function(object, ...){
		interaction <- matrix(1,nrow=nrow(object), ncol=nrow(object))
		res <- MizerParams_NPAC(object,interaction, ...)
		return(res)
})
