...
setMethod('getTempEffectRealm', signature(object='MizerParams_NPAC', ocean_temp='numeric'), # PW
	function(object, ocean_temp, ...){ # PW
		...
		unscaled_temp_effect <- ocean_temp * (ocean_temp - object@species_params$temp_min) * (object@species_params$temp_max - ocean_temp) * (exp(25.22 - (0.63/((8.62e-5)*(273+ocean_temp))))) # PW
		scaled_temp_effect <- unscaled_temp_effect / object@species_params$temp_scale
		out <- object@ontogenetic_migration # PW
		out[] <- scaled_temp_effect * c(object@exposure) * c(object@ontogenetic_migration) # PW
...
setMethod('getFeedingLevel', signature(object='MizerParams_NPAC', n = 'matrix', n_pp='numeric', ocean_temp='numeric', phi_prey='matrix'), # PW
	...
	# Check dims of phi_prey
		...
		encount <- temp_effect * object@search_vol * phi_prey # PW
		# encount <- object@search_vol * phi_prey # PW
...
n_pp='numeric', ocean_temp='numeric', phi_prey='matrix', feeding_level = 'matrix', pred_rate = 'array'), # PW
	function(object,n, n_pp, ocean_temp, phi_prey, feeding_level, pred_rate){ # PW
		...
		# Need to ensure no predation mortality on shark sizes that don't exist (PW)
		for (i in 1:nrow(object@species_params)){ # PW
			if (object@species_params$w_min_idx[i] != 1) # PW
			m2[i,1:object@species_params$w_min_idx[i]-1] = 0 # PW
		} # PW
...
n_pp = 'numeric', effort='numeric', m2 = 'matrix'),
	function(object, n, n_pp, effort, m2){
		...
		for (i in 1:nrow(object@species_params)){ # PW
			if (object@species_params$w_min_idx[i] != 1) # PW
			z[i,1:object@species_params$w_min_idx[i]-1] = 0 # PW
		} # PW
...
stop("feeding_level argument must have dimensions: no. species (",nrow(object@species_params),") x no. size bins (",length(object@w),")")
		...
		temp_effect <- getTempEffect(object, ocean_temp=ocean_temp) # PW
		e <- e - (temp_effect * object@std_metab) - (temp_effect * object@activity) # PW
		# e <- e - (object@std_metab) - (object@activity) # PW
