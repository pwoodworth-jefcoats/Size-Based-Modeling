# Plotting methods for the MizerSim_NPAC class
...
#' 
#' @param object An object of class \code{MizerSim_NPAC}.
...
#' sim <- project_NPAC(params, effort=1, t_max=20, t_save = 2)
#' plotBiomass(sim)
...
#' Plot the biomass using a \code{MizerSim_NPAC} object.
#' @rdname plotBiomass
setMethod('plotBiomass', signature(object='MizerSim_NPAC'),
	function(object, print_it=TRUE, start_time=as.numeric(dimnames(object@n)[[1]][1]), end_time = as.numeric(dimnames(object@n)[[1]][dim(object@n)[1]]), ...){
		...
		if (nrow(object@params@species_params)>14){
...
#' Plot the yield using a \code{MizerSim_NPAC} object.
#' @rdname plotYield
setMethod('plotYield', signature(object='MizerSim_NPAC'),
...
#' Plot the yield of each gear using a \code{MizerSim_NPAC} object.
#' @rdname plotYieldGear
setMethod('plotYieldGear', signature(object='MizerSim_NPAC'),
	function(object, print_it=TRUE, ...){
...
#' Plot the abundance spectra using a \code{MizerSim_NPAC} object.
#' @rdname plotSpectra
setMethod('plotSpectra', signature(object='MizerSim_NPAC'),
...
#' PLot the feeding level using a \code{MizerSim_NPAC} object.
#' @rdname plotFeedingLevel
setMethod('plotFeedingLevel', signature(object='MizerSim_NPAC'),
...
#' Plot M2 using a \code{MizerSim_NPAC} object.
#' @rdname plotM2
setMethod('plotM2', signature(object='MizerSim_NPAC'),
	function(object, time_range = max(as.numeric(dimnames(object@n)$time)), print_it = TRUE, ...){
...
#' Plot total fishing mortality using a \code{MizerSim_NPAC} object.
#' @rdname plotFMort
setMethod('plotFMort', signature(object='MizerSim_NPAC'),
	function(object, time_range = max(as.numeric(dimnames(object@n)$time)), print_it = TRUE, ...){
...
#' Plot total temperature effect on each species by size (PW)
#' 
#' After running a projection, plot the total temperature effect on each species
#' by size. The total temperature effect is averaged over the specified time
#' range (a single value for the time range can be used to plot a single time
#' step).
#' 
#' @param object An object of class \code{MizerSim_NPAC}.
...
#' sim <- project_NPAC(params, effort=1, t_max=20, t_save = 2)
#' plotTempEffect(sim)
#' plotTempEffect(sim, time_range = 10:20)
#' }
setGeneric('plotTempEffect', function(object, ...) # PW
	standardGeneric('plotTempEffect')) # PW

#' Plot total temperature effect using a \code{MizerSim_NPAC} object. (PW)
#' @rdname plotTempEffect
setMethod('plotTempEffect', signature(object='MizerSim_NPAC'), # PW
	function(object, time_range = max(as.numeric(dimnames(object@n)$time)), print_it = TRUE, ...){
		te_time <- getTempEffect(object, time_range=time_range, drop=FALSE, ...) # PW
		te <- apply(te_time, c(2,3), mean) # PW
		plot_dat <- data.frame(value = c(te), Species = dimnames(te)[[1]], w = rep(object@params@w, each=nrow(object@params@species_params))) # PW
		p <- ggplot(plot_dat) + geom_line(aes(x=w, y = value, colour = Species, linetype=Species)) + scale_x_continuous(name = "Size", trans="log10") + scale_y_continuous(name = "Total Temperature Effect", limits=c(0,max(plot_dat$value))) # PW
	if (nrow(object@params@species_params)>14){
		p <- ggplot(plot_dat) + geom_line(aes(x=w, y = value, group = Species)) + scale_x_continuous(name = "Size", trans="log10") + scale_y_continuous(name = "Total Temperature Effect", limits=c(0,max(plot_dat$value))) # PW
	}
	if (print_it)
		print(p)
		return(p)
...
#' Summary plot for \code{MizerSim_NPAC} objects
#' 
...
#' 
#' @param x An object of class \code{MizerSim_NPAC}
#' @param y Not used
...
#' }
setMethod("plot", signature(x="MizerSim_NPAC", y="missing"),
	function(x, ...){
		...
		p6 <- plotFMort(x,print_it = FALSE,...)
		p5 <- plotTempEffect(x,print_it = FALSE,...) # PW
		grid.newpage()
		...
		print(p6+ theme(legend.position="right", legend.key.size=unit(0.4,"cm")), vp = vplayout(3,1:2)) # PW
	}
)