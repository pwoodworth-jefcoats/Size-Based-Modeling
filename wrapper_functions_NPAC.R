...
#' 
#' This functions creates a \code{\linkS4class{MizerParams_NPAC}} object so that
...
#' \code{MizerParams_NPAC} object) is set to 0. Standard metabolism has been turned 
...
#' selected.
#' 
#' The resulting \code{MizerParams_NPAC} object can be projected forward using 
#' \code{project_NPAC()} like any other \code{MizerParams_NPAC} object. When projecting 
...
#' @param ... Other arguments to pass to the \code{MizerParams_NPAC} constructor.
#' @export
#' @return An object of type \code{\linkS4class{MizerParams_NPAC}}
...
#' sim <- project_NPAC(params, effort = 0, t_max = 100, dt=0.1)
#' plotBiomass(sim)
#' plotSpectra(sim)
#' }
set_community_model <- function(max_w = 1e6,
	...
	com_params <- MizerParams_NPAC(com_params_df, p=p, n=n,q=q, lambda = lambda, 
...
#' 
#' This functions creates a \code{MizerParams_NPAC} object so that trait-based-type 
...
#' \code{mizerNPAC}. All the species-specific parameters are the same, except for
...
#' Pedersen (2010). See the \code{mizerNPAC} vignette for more details and examples
...
#' @seealso \linkS4class{MizerParams_NPAC}
...
#' sim <- project_NPAC(trait_params, effort = 0, t_max = 50, dt=0.2,
...
#' sim <- project_NPAC(params_gear, t_max = 20, effort = c(Industrial = 1, Other = 0))
#' }
set_trait_model <- function(no_sp = 10,
	...
	# Make the MizerParams_NPAC
	trait_params <- MizerParams_NPAC(trait_params_df, min_w = min_w, max_w=max_w, no_w = no_w, min_w_pp = min_w_pp, w_pp_cutoff = w_pp_cutoff, n = n, p=p, q=q, r_pp=r_pp, kappa=kappa, lambda = lambda) 
